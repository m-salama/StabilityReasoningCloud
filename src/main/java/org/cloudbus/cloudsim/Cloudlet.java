...
public class Cloudlet {
	...
	/**
	 * The input file size of this Cloudlet before execution (unit: in byte). in byte = program + input data size
	 */
	 ...
	 * 
	 * @param cloudletLength the length or size (in MI) of this Cloudlet to be executed in a CloudResource
	 ...
	 * @post $result >= 0.0
	 */
	public double getTotalCost(List<? extends Vm> vmlist) {
		...
		cost += costPerCPU * getActualCPUTime();
		
		Vm vm = vmlist.get(this.vmId);
		cost+= costPerMemory * vm.getRam();
		//cost+= costPerStorage * vm.getSize();

		return cost;
	}

	public double getProcessingCost(List<? extends Vm> vmlist) {
		// cloudlet cost: execution cost...
		//double cost = getProcessingCost();
		double cost = 0;
